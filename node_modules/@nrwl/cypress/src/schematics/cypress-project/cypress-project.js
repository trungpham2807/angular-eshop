...
function generateFiles(options) {
    return () => {
        return schematics_1.mergeWith(schematics_1.apply(schematics_1.url('./files'), [
            schematics_1.template(Object.assign(Object.assign({ tmpl: '' }, options), { ext: options.js ? 'js' : 'ts', offsetFromRoot: devkit_1.offsetFromRoot(options.projectRoot) })),
...
function normalizeOptions(host, options) {
    ...
    const projectRoot = options.directory
        ? core_1.join(core_1.normalize(ast_utils_1.appsDir(host)), devkit_1.names(options.directory).fileName, options.name)
        : core_1.join(core_1.normalize(ast_utils_1.appsDir(host)), options.name);
    return Object.assign(Object.assign({}, options), { projectName,
        projectRoot });
}
exports.cypressProjectGenerator = ngcli_adapter_1.wrapAngularDevkitSchematic('@nrwl/cypress', 'cypress-project');
