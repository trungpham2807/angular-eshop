...
function printAffected(affectedProjectsWithTargetAndConfig, affectedProjects, projectGraph, { nxJson }, nxArgs, overrides) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        const { tasksOptions } = run_command_1.getRunner(nxArgs, nxJson, Object.assign(Object.assign({}, nxArgs), overrides));
        const projectNames = affectedProjects.map((p) => p.name);
        const hasher = new hasher_1.Hasher(projectGraph, nxJson, tasksOptions);
        const tasksJson = yield createTasks(affectedProjectsWithTargetAndConfig, projectGraph, nxArgs, overrides, hasher);
        const result = {
            tasks: tasksJson,
...
function createTasks(affectedProjectsWithTargetAndConfig, projectGraph, nxArgs, overrides, hasher) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        const taskHashes = yield hasher.hashTasks(tasks);
        const isYarn = path_1.basename(process.env.npm_execpath || 'npm').startsWith('yarn');
        return tasks.map((task, index) => ({
            id: task.id,
            overrides: overrides,
            target: task.target,
            command: `${isYarn ? 'yarn' : 'npm run'} ${utils_1.getCommandAsString('nx', isYarn, task)}`,
