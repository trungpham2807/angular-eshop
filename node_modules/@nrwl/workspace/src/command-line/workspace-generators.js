...
function workspaceGenerators(args) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        if (ws.isNxGenerator(collectionFile, generatorName)) {
            try {
                child_process_1.execSync(`npx tao g "${collectionFile}":${generatorName} ${args
                    .slice(1)
                    .join(' ')}`, { stdio: ['inherit', 'inherit', 'inherit'] });
            }
            catch (e) {
                process.exit(1);
            }
...
function constructCollection() {
    ...
    fs.readdirSync(generatorsDir()).forEach((c) => {
        ...
        if (exists(path.join(childDir, 'schema.json'))) {
            generators[c] = {
                factory: `./${c}`,
                schema: `./${core_1.normalize(path.join(c, 'schema.json'))}`,
...
function saveCollection(dir, collection) {
    fs_1.writeFileSync(path.join(dir, 'workspace-generators.json'), JSON.stringify(collection));
}
...
function listGenerators(collectionName, logger) {
    try {
        ...
        bodyLines.push('');
        bodyLines.push(...Object.entries(collection.description.schematics).map(([schematicName, schematicMeta]) => {
        ...
        workflow.reporter.subscribe((event) => {
            ...
            switch (event.kind) {
                case 'error':
                    ...
                    break;
                case 'update':
                    loggingQueue.push(core_1.tags.oneLine `${chalk.white('UPDATE')} ${eventPath} (${event.content.length} bytes)`);
                    break;
                case 'create':
                    loggingQueue.push(core_1.tags.oneLine `${chalk.green('CREATE')} ${eventPath} (${event.content.length} bytes)`);
                    break;
                case 'delete':
                    loggingQueue.push(core_1.tags.oneLine `${chalk.yellow('DELETE')} ${eventPath}`);
                    break;
                case 'rename':
                    const eventToPath = event.to.startsWith('/')
                        ? event.to.substr(1)
                        : event.to;
                    loggingQueue.push(core_1.tags.oneLine `${chalk.blue('RENAME')} ${eventPath} => ${eventToPath}`);
                    break;
            }
        });
